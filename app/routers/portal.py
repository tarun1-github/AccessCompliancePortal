...
def home():
    ...
    return HTMLResponse(
        ...
        <html>
        <head>
            <title>Access Compliance Portal</title>
    ...
    return HTMLResponse(
        ...
        <p><a href="/{current_user.alias}">Open your portal</a></p>
        """
    )
...
#
# Phase 2A:
# The old token URL must not bypass authentication.
# Token-based first-time password bootstrap will be added
# separately. For now an authenticated session is required.
# ============================================================
...
def verify_engineer(
    ...
    # A verification token must never allow one engineer
    # to view another engineer's portal.
    if (
        str(current_user.role or "").upper() != "SUPERVISOR"
        and current_user.id != engineer.id
    ):
        raise HTTPException(
            status_code=403,
...
# SUPERVISOR: can view engineer data
# ============================================================
...
def get_engineer_details(
    ...
    if (
        ...
        raise HTTPException(
            status_code=403,
            detail="You are not authorized to access another engineer's data.",
        )
...
def analytics_page(
    ...
    # Never trust supervisor_id from the URL.
    if current_user.id != supervisor_id:
        raise HTTPException(
            status_code=403,
            detail="You are not authorized to access this supervisor's analytics.",
        )
...
def analytics_csv(
    ...
    if current_user.id != supervisor_id:
        raise HTTPException(
            status_code=403,
            detail="You are not authorized to export this supervisor's analytics.",
        )
...
def engineer_alias_page(
    ...
    #
    # Normal engineer -> own page only
    # Supervisor       -> can view engineer pages
    # ========================================================
